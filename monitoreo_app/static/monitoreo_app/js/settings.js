...
window.abrirModalUsuario = async function(id = null) {
    ...
    // Resetear visibilidad de los campos sensibles
    const tokenInput = document.getElementById('usuarioTelegramToken');
    const chatIdInput = document.getElementById('usuarioTelegramChatId');
    const eyeToken = document.getElementById('eyeToken');
    const eyeChatId = document.getElementById('eyeChatId');
    
    if (tokenInput) tokenInput.type = 'password';
    if (chatIdInput) chatIdInput.type = 'password';
    if (eyeToken) {
        eyeToken.classList.remove('fa-eye-slash');
        eyeToken.classList.add('fa-eye');
    }
    if (eyeChatId) {
        eyeChatId.classList.remove('fa-eye-slash');
        eyeChatId.classList.add('fa-eye');
    }
