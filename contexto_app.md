...
- Dockerfile con Python 3.12 para la aplicación.
- docker-compose.yml con servicios: web, celery_worker, celery_beat, redis, db.
- Entrypoint para esperar la base de datos, ejecutar migraciones y recolectar estáticos.
- Volúmenes para persistencia de datos y archivos estáticos.
- Redes aisladas para comunicación entre servicios.
...
Contenerización: Docker y Docker Compose para facilitar el despliegue en cualquier entorno.
...
# Ver ejemplo en la sección de Variables de Entorno
...
# 6. Terminal 1 - Celery Worker
celery -A monitoreo_proyecto worker --loglevel=info --pool=solo

# 7. Terminal 2 - Celery Beat
celery -A monitoreo_proyecto beat --loglevel=info

# 8. Terminal 3 - Servidor Django
